...
const mtx: Color[][] = Array.from({ length: amountRows }, ee => Array.from({ length: amountColumns }, vv => colors[Math.floor(Math.random() * colors.length)]));

function BFS(mtx: Color[][], row: number, column: number): void {
    const visit: boolean[][] = Array.from({ length: amountRows }, ee => Array.from({ length: amountColumns }, vv => false));
    ...
    mtx[row][column] = (colorInput as HTMLInputElement).value as Color;
    ...
    while(q.length !== 0) {
        ...
        for(let k = 0; k < directions.length; k++) {
            ...
            if(
                ...
                mtx[xRef][yRef] = (colorInput as HTMLInputElement).value as Color;
            }
        }
    }
